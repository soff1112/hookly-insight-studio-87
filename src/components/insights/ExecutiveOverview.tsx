...
import { TrendingUp, TrendingDown, Eye, Heart, Users, ArrowUpRight, Lightbulb, ChevronRight } from "lucide-react";
interface KPIMetric {
  label: string;
  value: string;
  change: number;
  ...
  return <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Aggregated performance snapshot  </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {kpiMetrics.map((metric, index) => <Card key={index} className="p-4 border-border">
            <div className="flex items-start justify-between">
              <div className="p-2 rounded-lg bg-primary/10 text-primary">
                {metric.icon}
              </div>
              <div className="flex items-center gap-1 text-xs">
                {metric.change >= 0 ? <TrendingUp className="w-3 h-3 text-accent" /> : <TrendingDown className="w-3 h-3 text-destructive" />}
            ...
            <div className="mt-3 pt-3 border-t border-border">
              <div className="flex items-center justify-between text-xs">
                ...
                {metric.vsCompetitor && <Badge variant="secondary" className={`text-[10px] px-1.5 py-0 ${metric.vsCompetitor.includes("+") || metric.vsCompetitor.includes("above") ? "bg-accent/10 text-accent" : metric.vsCompetitor.includes("-") || metric.vsCompetitor.includes("below") ? "bg-destructive/10 text-destructive" : "bg-muted"}`}>
                    {metric.vsCompetitor}
                  </Badge>}
              </div>
            </div>
      ...
      <Card className="p-5 border-border bg-gradient-to-br from-primary/5 to-transparent">
        ...
        <div className="space-y-3">
          {topInsights.map((insight, index) => <div key={index} className={`p-4 rounded-lg border ${priorityStyles[insight.priority]} cursor-pointer hover:shadow-sm transition-shadow`}>
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    <Badge variant="secondary" className={`text-[10px] px-1.5 py-0 ${priorityStyles[insight.priority]}`}>
